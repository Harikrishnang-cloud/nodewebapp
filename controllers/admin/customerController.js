...
const customerBlocked = async(req,res)=>{
    try {
        ...
        res.status(statusCode.success).json({success:true, message:"user update successful"});
    } catch (error) {
        console.error("Error in customerBlocked:", error);
        res.status(statusCode.Internal_server_error).json({success:false, message:"Failed to update user status"});
    }
};
...
const searchCustomer = async(req,res)=>{
    try {
        ...
        res.status(statusCode.Internal_server_error).json({ success: false, message: "Search failed" });
    }
};
